<template>
  <div class="">
    <h4>Рейсы</h4>
    <ul class="list-group">
      <li class="list-group-item" :class="{ active: index == currentIndex }" v-for="(flight, index) in flights"
        :key="index" @click="setActiveFlight(flight, index)">
  ...
  <div class="col-md-6">
    <div v-if="currentFlight">
      <h4>Flight</h4>
      <div>
        <label><strong>Время отлёта:</strong></label> {{ currentFlight.startDate }}
      </div>
      <div>
        <label><strong>Время прилёта:</strong></label> {{ currentFlight.endDate }}
      </div>
      <div>
        <label><strong>Место отлёта:</strong></label> {{ currentFlight.startCity }}
      </div>
      <div>
        <label><strong>Место прилёта:</strong></label> {{ currentFlight.endCity }}
      </div>1
